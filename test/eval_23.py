...
def validate_view_first_order_detail(result=None,device_id=None,backup_dir=None):
    ...
    for record in all_data:
        if record.get('action') == 'navigate' and record.get('page') == 'order_detail':
            ...
            if from_page == 'my_orders' and (order_index == 2 or is_first_order) and order_status == '已送达':
